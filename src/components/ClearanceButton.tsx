...
// ── SHA-256 constants and rotR helper for PoW optimization ──
const K_POW = new Uint32Array([
  0x428a2f98,0x71374491,0xb5c0fbcf,0xe9b5dba5,0x3956c25b,0x59f111f1,0x923f82a4,0xab1c5ed5,
  0xd807aa98,0x12835b01,0x243185be,0x550c7dc3,0x72be5d74,0x80deb1fe,0x9bdc06a7,0xc19bf174,
  0xe49b69c1,0xefbe4786,0x0fc19dc6,0x240ca1cc,0x2de92c6f,0x4a7484aa,0x5cb0a9dc,0x76f988da,
  0x983e5152,0xa831c66d,0xb00327c8,0xbf597fc7,0xc6e00bf3,0xd5a79147,0x06ca6351,0x14292967,
...
export default function ClearanceButton({ appId, status, variant = 'default' }: ClearanceButtonProps) {
  ...
  const solveVerification = async (nonce: string, difficulty: string): Promise<string> => {
    // If the attempt is too long, fall back to slow but generic sha256 to be 100% robust
    if (nonce.length + 10 > 55) {
      let n = 0;
      ...
      while (true) {
        ...
        if (n > 5000000) throw new Error("Mathematical limit exceeded.");
      }
    }

    const diffLen = difficulty.length;
    let expectedH0Prefix = 0;
    let h0Mask = 0;
    for (let i = 0; i < Math.min(diffLen, 8); i++) {
      const char = difficulty[i];
      const val = parseInt(char, 16);
      h0Mask |= (0xF << (28 - i * 4));
      expectedH0Prefix |= (val << (28 - i * 4));
    }
    ...
    while (true) {
      ...
      for (let i = 0; i < len; i++) {
        words[i >> 2] |= attempt.charCodeAt(i) << (24 - (i % 4) * 8);
      }
      words[len >> 2] |= 0x80 << (24 - (len % 4) * 8);
      words[15] = len * 8;
      ...
      for (let j = 0; j < 16; j++) {
        const t2 = (rotR_pow(a, 2) ^ rotR_pow(a, 13) ^ rotR_pow(a, 22)) + ((a & b) ^ (a & c) ^ (b & c));
        const t1 = hh + (rotR_pow(e, 6) ^ rotR_pow(e, 11) ^ rotR_pow(e, 25)) + ((e & f) ^ (~e & g)) + K_POW[j] + words[j];
        hh = g; g = f; f = e; e = (d + t1) | 0; d = c; c = b; b = a; a = (t1 + t2) | 0;
      }
      for (let j = 16; j < 64; j++) {
        const w15 = words[j-15], w2 = words[j-2], w16 = words[j-16], w7 = words[j-7];
        const s0 = rotR_pow(w15, 7) ^ rotR_pow(w15, 18) ^ (w15 >>> 3);
        const s1 = rotR_pow(w2, 17) ^ rotR_pow(w2, 19) ^ (w2 >>> 10);
        words[j] = (w16 + s0 + w7 + s1) | 0;
      ...
      if ((h[0] & h0Mask) === expectedH0Prefix) {
        h[1] = (h[1] + b) | 0;
        h[2] = (h[2] + c) | 0;
        h[3] = (h[3] + d) | 0;
        h[4] = (h[4] + e) | 0;
        ...
        const res: string[] = [];
        for (let i = 0; i < 8; i++) {
          res.push((h[i] >>> 0).toString(16).padStart(8, '0'));
        }
        const fullHex = res.join('');
        if (fullHex.startsWith(difficulty)) {
          return n.toString();
        }
      }
